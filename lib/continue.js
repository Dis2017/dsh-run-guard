...
export function applyContinue(ctx, config) {
  ...
  ctx.on('session/event', (session, event) => {
    ...
    if (t === 'todo/write') {
      ...
      if (enabled) ctx.logger?.info(`[dsh-run-guard][continue] assistant/message T${event.data.turn}/S${event.data.step}`);
      ...
      if (enabled) ctx.logger?.info(`[dsh-run-guard][continue] turn/end T${event.data.turn} kind=${event.data.reason?.kind}`);
