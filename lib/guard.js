...
export function applyGuardRecovery(ctx, config) {
  ...
  ctx.on('agent/request-error', async (payload, next) => {
    ...
    ctx.logger?.info(
      `[dsh-run-guard][recovery] request-error code=${code} turn=${payload.turn} step=${payload.step} agent=${payload.agent?.id} retryable=${typeof code === 'string' && retryable.has(code)}`
    );
